            ...
            if rank == 0:
                if args.no_eval:
                    ...
                    if not args.nolog:
                        ...
                        wandb.log({"3d_train": losses_3d_train[-1] * 1000, "3d_train_eval": losses_3d_train_eval[-1] * 1000, "3d_valid": losses_3d_valid[-1] * 1000, "3d_val_velocity": epoch_loss_3d_vel * 1000}, epoch+1)
                if not args.nolog:
    ...
    def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False, newmodel=None):
        ...
        with torch.no_grad():
            ...
            for cam, batch, batch_2d in test_generator.next_epoch():
                ...
                # gt2d = project_to_2d_linear(inputs_3d + inputs_traj, cam)
                # pred_traj, _ = get_root(predicted_3d_pos, inputs_2d, cam)
                # pred_p2d = project_to_2d_linear(predicted_3d_pos + inputs_traj, cam)
                # print(abs(inputs_2d - gt2d).mean() * 1000)
                # print(abs(pred_p2d - gt2d).mean() * 1000)
                # print(abs(pred_traj - inputs_traj.squeeze()).mean() * 1000)
                # exit()
        ...
        if rank == 0:
            ...
            print('Protocol #2 Error (A-MPJPE):', e3, 'mm')
