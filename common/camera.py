...
def camera_to_NDC(X, camera_params):
    """
    ...
    """
    proj_2d = project_to_2d(X, camera_params)
    X_25D = torch.stack((proj_2d[..., 0], proj_2d[..., 1], X[..., 2]), dim = -1)
    
    return X_25D    

def NDC_to_camera(X, camera_params):
    """
    Project 3D points to 2D using the Human3.6M camera projection function.
    This is a differentiable and batched reimplementation of the original MATLAB script.
...
def project_to_2d_with_root(X, camera_params, root):
    """
    Project 3D points to 2D using the Human3.6M camera projection function.
    This is a differentiable and batched reimplementation of the original MATLAB script.
